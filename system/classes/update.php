...
//                    }
        ...
        public function fetchFiles(object $db, string $currentVersion, string $updateVersion, array $lang): void
        {
            ...
            if (file_exists($basedir.$this->localUpdateSystemPath . $this->updateFilesFile))
            {
                ...
                $updateFile = file_get_contents('https://update.yawk.io/filebase.ini');
                if (!$updateFile) {
                    die("Failed to fetch filebase.ini from update server!");
                }
                $this->updateFiles = parse_ini_string($updateFile, true);
