...
$(document).ready(function()
{
    ...
    let showOpenBtnPositionThreshold = 600;      // position from where the 'open sidebar btn' gets shown (in pixels)
    ...
    sidebar.hide();     // hide the sidebar on page load

    // which element should be used to determine, if the sidebar should be hidden, because it overlaps with another element?
    let overlapPositionOrDivBox = $('#globalmenu'); // use globalmenu as overlap indicator to hide sidebar

    // Get the current scroll position
        $(window).scroll(function()
        {   // get position of overlap element
            let overlapPosition = overlapPositionOrDivBox.offset().top + overlapPositionOrDivBox.outerHeight();
            // get the bottom position of the sidebar element
            let sidebarCollider = sidebar.offset().top + sidebar.outerHeight();
            // store the sidebar element in a variable
            let thisSidebar = document.getElementById("mySidebar");
